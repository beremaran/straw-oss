  ...
  afterRender(state) {
    ...
    const renderPoolTiers = () => {
      poolsContainer.innerHTML = formState.endpoint_pools
        .map((pool, idx) => {
          const epOptions = (state.endpointsData || [])
            .map((ep) => {
    ...
    const renderAbVariants = () => {
      ...
      abVariantsContainer.innerHTML = formState.ab_variants
        .map((v, idx) => {
          const presetOptions = presetsList
            .map(
              (p) => `
    ...
    const getPayloadFromInputs = () => {
      ...
      const payload = {
        ...
        endpoint_pools: formState.endpoint_pools.filter(
          (p) => p.endpoint_ids && p.endpoint_ids.length > 0
        ),
        hard_timeout,
    ...
    const inputsToBind = [
      ...
      'rule-limit-min',
      'rule-limit-sec',
      'rule-fp-preset-select',
      'rule-ab-strategy',
      'rule-filters-adblock',
      'rule-filters-lists',
