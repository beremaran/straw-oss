...
export function validateScope(scopeStr) {
  ...
  if (trimmed.includes('*')) {
    ...
    throw new Error(
      `Invalid scope wildcard format '${trimmed}'. Only key:* or *:value wildcards are supported`
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Invalid scope format '${trimmed}'. Must be *, prefix wildcard (e.g. target:*), suffix wildcard (e.g. *:us), or exact tag (e.g. region:us)`
    )
  }
}
...
export function validateDuration(durationStr) {
  ...
  if (!goDurationRegex.test(trimmed)) {
    throw new Error(
      "Invalid Go duration (e.g. '500ms', '30s', '1m', '2h45m'). Natural language values like '30 seconds' are rejected."
    )
  }
  return true
}
