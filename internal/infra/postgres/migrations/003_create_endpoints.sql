...
COMMENT ON TABLE endpoints IS 'Registered endpoint workers';
COMMENT ON COLUMN endpoints.id IS 'Unique endpoint identifier, e.g. "endpoint-us-res-001"';
COMMENT ON COLUMN endpoints.tags IS 'Endpoint capabilities, e.g. ["type:residential", "region:us"]';
COMMENT ON COLUMN endpoints.last_heartbeat IS 'Last heartbeat timestamp for health detection';
COMMENT ON COLUMN endpoints.metadata IS 'Additional metadata like version, IP, etc.';
-- +goose StatementEnd
