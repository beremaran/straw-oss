...
COMMENT ON TABLE usage_records IS 'Hourly usage records for billing and analytics';
COMMENT ON COLUMN usage_records.quota_key IS 'Tag-derived quota key, e.g. "target:amazon"';
COMMENT ON COLUMN usage_records.endpoint_tier IS 'Endpoint type: residential, datacenter, mobile';
COMMENT ON COLUMN usage_records.period_start IS 'Start of the hourly bucket';
COMMENT ON COLUMN usage_records.period_end IS 'End of the hourly bucket';
-- +goose StatementEnd
