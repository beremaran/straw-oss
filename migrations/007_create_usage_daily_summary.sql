...
COMMENT ON TABLE usage_daily_summary IS 'Pre-aggregated daily usage summaries for billing';
COMMENT ON COLUMN usage_daily_summary.cost_units IS 'Total cost units weighted by endpoint type multipliers';
COMMENT ON COLUMN usage_daily_summary.breakdown IS 'Request count breakdown by endpoint type';
-- +goose StatementEnd
