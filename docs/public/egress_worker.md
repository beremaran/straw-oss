...
| **capabilities.supported_ingress_modes**| `array` | No | List of supported proxy styles. Valid values: `rest`, `http_proxy`, `connect`, `mitm` (default `["rest"]`). See [Proxy & Ingress Modes](ingress_modes.md) for details. When `mitm` is enabled, clients fetch and rotate the worker's TLS-terminating CA cert via the Control plane's [MITM CA Distribution API](api/config.md#13-mitm-ca-distribution). |
