...
1. **Target URL**:
   ...
   - Hostnames are normalized using Punycode/IDNA if they contain non-ASCII characters.
   - Default port components are stripped for routing rule matching (`http://host:80` -> `http://host`), but the original host name is preserved for sending.
2. **HTTP Headers**:
   ...
   - Client-supplied `Host`, `Content-Length`, `Transfer-Encoding`, and any hop-by-hop headers declared in the `Connection` header are stripped. The Egress worker generates the outbound `Host` and `Content-Length` dynamically.
3. **HTTP Method**:
...
- **body.mode**: Always `inline_base64` in P0.
