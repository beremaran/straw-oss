...
| P0 exclusions | capture_hint other than `none` rejected; redirect flag rejected; protobuf-defined BodyRef rejected in P0       |
...
| Auth          | platform key lifecycle, platform key cannot execute requests, tenant key cannot create tenants, revocation     |
...
| Worker admin  | tenant worker actions affect only that tenant; global worker actions require system_admin                     |
